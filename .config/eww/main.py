        ...
        string = string + f"""
                            (button :class 'notif'
                                (box :orientation 'h' :space-evenly false
                                    ...
                                    (box :orientation 'v' :halign 'start' :valign 'start' :hexpand true 
                                        (label :class "title_yes" :width 200 :truncate true :wrap true :text '{item.summary or ''}')
                                        (label :width 200 :truncate false :wrap true :text '{item.body or ''}')
                                        )))
                          """
    string = string.replace('\n', ' ')
    print(fr"""(box :orientation 'v' {string or ''})""", flush=True)
